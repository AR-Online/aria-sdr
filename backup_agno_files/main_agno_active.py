# main_agno_active.py - ARIA-SDR com AgentOS ativo para Agno
...
# Create FastAPI app
app = FastAPI(title="ARIA-SDR AgentOS Active", debug=DEBUG)
...
try:
    ...
    # Configure AgentOS with proper setup
    agent_os = AgentOS(
...
def healthz():
    return {
        ...
        "status": "active" if AGENTOS_AVAILABLE else "inactive"
    }
...
def root():
    ...
    return {
        "message": "ARIA-SDR AgentOS is running",
        "status": "active",
        "agentos": AGENTOS_AVAILABLE,
        "version": "1.0.0"
    }
...
def get_agentos_routes():
    ...
    if not AGENTOS_AVAILABLE:
        return {"error": "AgentOS nÃ£o disponÃ­vel", "status": "inactive"}
...
def agno_status():
    ...
    return {
        "status": "active",
        "agentos": AGENTOS_AVAILABLE,
        "message": "ARIA-SDR AgentOS is ready for Agno"
    }

@app.post("/agno/webhook")
def agno_webhook(request: Request):
    """Agno webhook endpoint"""
    try:
        body = request.json()
        return {
            "status": "received",
            "message": "Webhook received from Agno",
            "data": body
        }
    except Exception as e:
        return {
